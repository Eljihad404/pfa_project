...
def register_user(payload: UserCreate, db: Session = Depends(get_db)):
    ...
    if not role_user:
        ...
        db.add(role_user); db.commit(); db.refresh(role_user)
    db.add(UserRole(user_id=user.id, role_id=role_user.role_id)); db.commit()
...
def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    roles = get_user_roles(db, user.id)
    # include roles in token (optional), but sub is user.id for stability
